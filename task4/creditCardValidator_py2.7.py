...
def is_card_valid(card):
    ...
    res_start_with_exact, content_start_with_exact = \
        is_start_with_exact(card, ('4', '5', '6'))
    res_consecutive, content_consecutive = is_consecutive(card)
    card = original_card if original_card else card
    if res_only_digits and res_contain16digits \
            and res_start_with_exact and res_consecutive:
        print("Credit card: {} is valid".format(card))
    else:
        print("Credit card: {} {} {} {} {}  Invalid"
              .format(card, content_only_digits, content_contain16digits,
    ...
    if len(card) > 1:
        for i in range(1, len(card)):
            if card[i - 1] == card[i]:
                if count >= 4:
                    return False, \
                           "Includes consecutive digits" \
                           " are repeating 4 or more times;"
                count += 1
            else:
                count = 1
    return True, ""
